...
namespace Lux.Domain
{
    public readonly partial struct TimeOfDay
    {
        ...
        {
            //DateTime time = DateTime.Today.Add(_offset);
            //string displayTime = time.ToString("t"[, CultureInfo.CurrentCulture]);
            //return base.ToString();
            return $"{_offset.Hours.ToString("00")}:{_offset.Minutes.ToString("00")}";
        }
